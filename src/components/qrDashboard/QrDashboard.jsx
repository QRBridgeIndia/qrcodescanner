...
function QrDashboard() {
  ...
  return (
    <div className="flex flex-col px-5 pt-5 w-full pb-[538px] max-md:pb-24 max-md:max-w-full">
      ...
      className={`flex flex-col justify-center items-center py-4 w-full text-sm font-medium tracking-normal leading-snug text-blue-900 bg-[#DFF2FF] rounded max-md:px-5 max-md:max-w-full mb-5`}
...
<div className="grow shrink my-auto whitespace-nowrap" style={{ color: "#0077B6" }}>
  Scan QR Code
      ...
      {features.map((feature, index) => (
        <div key={feature.text} className={index > 0 ? "mt-5 flex flex-col items-center" : ""}>
          <QrFeatureCard
            icon={feature.icon}
            text={feature.text}
